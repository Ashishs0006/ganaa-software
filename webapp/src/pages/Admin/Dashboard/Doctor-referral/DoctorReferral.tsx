...
console.log('✌️data --->', data);
  ...
  const handleEditUserFunction = (user: IDoctor) => {
    if (user) {
      setData((prev) => ({
        ...prev,
        _id: user._id,
  ...
  const handleResetPassword = async (id: string) => {
    try {
      const response = await resetDoctorPassword(id);
      if (response.data.status == "success") {
      ...
      <div className="w-[1036px]!">
        <div className=" bg-white w-full gap-6 flex-col rounded-xl flex items-start p-4 min-h-[500px]!">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="bg-white w-full h-full">
                {doctorsData?.data?.length >= 1 &&
                  doctorsData?.data?.map((doctor:any, index:any) => (
                    <tr
                      key={index}
                      className={` border-b border-[#DCDCDCE0] ${
                        doctor.isDeleted ? "bg-gray-200" : "hover:bg-[#F6F6F6C7]"
                      } `}
                    >
                      ...
                      <td className="py-3 px-3 w-1/7">
                        <div
                          ...
                          title={doctor.centerId.map((data) => data.centerName).join(", ")}
                        >
                          {doctor.centerId.map((data) => data.centerName).join(", ")}
                        </div>
                      </td> */}
                      <td className="py-3 px-3 w-1/7">
                        <div
                                    ...
                                    <div className="p-2  text-nowrap whitespace-nowrap gap-2 flex-col flex justify-center items-start bg-white shadow-lg rounded-lg w-fit">
                                      ...
                                      <div
                                        onClick={() => {
                                          ...
                                          setData((prev) => ({ ...prev, _id: doctor._id }));
                                        }}
          ...
          <div className="w-full flex items-end justify-end">
            <Pagination totalPages={doctorsData?.pagination?.totalPages} />
          </div>
        </div>
      </div>
      <Modal
      ...
      >
        <div className="p-8">
          <form
            ...
            autoComplete="off"
          >
            <p className="font-bold text-[18px] mb-5">Add Doctor</p>
