...
//       centersFilter = {
//         $or: [
//           { centerId: { $in: centersArray } },
//           { centerId: { $exists: false } }, // case1: field missing
//           { centerId: null }                 // case2: field explicitly null
//         ]
//       };
...
export const getAllLeads = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    if (req.query.centers && typeof req.query.centers === "string") {
      ...
      // Case: Only one center selected -> strict match only
      if (centersArray.length === 1) {
        centersFilter = { centerId: centersArray[0] };
      } 
      // Case: Multiple centers selected -> include null + missing values also
      else if (centersArray.length > 1) {
        centersFilter = {
          $or: [
            { centerId: { $in: centersArray } },
