...
export const updateMe = catchAsync(async (req: UserRequest, res: Response, next: NextFunction) => {
  ...
  if (filterObj.hasOwnProperty('email')) {
    const emailCheck = await Doctor.exists({ email: filterObj.email });
  ...
  let updatedUser = await Doctor.findByIdAndUpdate(req!.user!._id, filterObj, {
    new: true,
...
export const changeMePassword = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    if (!user) return next(new AppError('Invalid Logged Doctor', 400));
...
export const getAllBasicDoctors = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    const filteredQuery = new FilterObject(req.query, 'centerId', 'page', 'sort', 'limit')
      .parseDate('dob')
      .get();
...
export const getAllDoctors = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    const features = new APIFeatures<IDoctor>(Doctor.find({isDeleted:false}), req.query)
      .filter()
      .sort()
      .limitFields()
      .paginate();
    ...
    const rawQuery = features.rawQuery();
    const data = await features.query;
    const paginationInfo = await PaginationInfo.exec(Doctor.countDocuments({isDeleted:false}), req.query, rawQuery);
...
export const createNewDoctors = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    if (filteredBody.hasOwnProperty('email')) {
      const emailCheck = await Doctor.exists({ email: filteredBody.email });
      if (emailCheck) return next(new AppError('Email Already Exists', 400));
    }

    let data: IDoctor | null = await Doctor.create({
      ...filteredBody,
      isDeleted: false,
    ...
    if (req.file) {
      ...
      await S3.uploadFile(filePath, req.file?.buffer, req.file?.mimetype);
      await Doctor.findByIdAndUpdate(data._id, { profilePic: filePath });
      data = await Doctor.findById(data._id);
    }
...
export const getSingleDoctor = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    if (!user) return next(new AppError('Invalid Doctor ID', 400));
...
export const updateDoctorInformation = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    if (!req.params.id) return next(new AppError('Id is Mandatory', 400));

    const isUserExist = await Doctor.exists({ _id: req.params.id }).lean();
    if (!isUserExist) return next(new AppError('Please provide valid Doctor Id', 400));
    ...
    let updatedUser = await Doctor.findByIdAndUpdate(req.params.id, filterObj, {
      new: true,
    });
...
export const deleteDoctor = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    await Doctor.findByIdAndUpdate(req.params.id, { isDeleted: true });
...
export const resetDoctorPassword = catchAsync(
  async (req: UserRequest, res: Response, next: NextFunction) => {
    ...
    const user = await Doctor.findOne({ _id: req.params.id });
    if (!user) return next(new AppError('Please provide valid Doctor Id', 400));
